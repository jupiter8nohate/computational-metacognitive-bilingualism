...
def _looks_like_repository_root(path: Path) -> bool:
    return (
        ...
        and (path / "src/cmb_agents/steward.py").is_file()
        and (path / "src/cmb_glitch8/glyphs.v1.json").is_file()
    )
...
class AuditReport:
    ...
    evidence_packets: tuple[dict[str, Any], ...] = ()
...
def _write_public_status(report: AuditReport) -> None:
    """Publish deterministic human/machine views of the latest steward audit."""
    generated = ROOT / "docs" / "generated"
    generated.mkdir(parents=True, exist_ok=True)

    payload = {
        "schema_version": "cmb.steward-status.v1",
        "overall_ok": report.ok,
        "roles": list(report.roles),
        "checks": [
            {"role": check.role, "name": check.name, "ok": check.ok}
            for check in report.checks
        ],
        "evidence_packets": list(report.evidence_packets),
        "boundaries": [
            "GENERATED != VERIFIED",
        ...
        "external_gate_note": (
            "This status reflects repository-side checks only. GitHub platform settings, "
            "independent review, and DOI/archive state require their own evidence."
        ),
    }
    ...
    lines = [
        ...
        f'<div class="cmb-status-led cmb-status-led--{"pass" if report.ok else "fail"}"><strong>SYSTEM</strong><span>{"PASS" if report.ok else "ATTENTION"}</span></div>',
...
def run_audit(*, include_docs: bool = True) -> AuditReport:
    ...
    for audit in specialist_audits:
        checks.append(
            CheckResult(
                role=audit.name,
                name=audit.packet.task,
                command=[],
                returncode=0 if audit.ok else 1,
                output=audit.summary,
            )
        )

    provisional = AuditReport(
        roles=ROLE_NAMES,
        checks=tuple(checks),
        generated_changes=(),
        evidence_packets=tuple(audit.packet.to_dict() for audit in specialist_audits),
    )
    _write_public_status(provisional)

    return AuditReport(
        roles=ROLE_NAMES,
        checks=tuple(checks),
        generated_changes=_git_changed_paths(),
        evidence_packets=provisional.evidence_packets,
    )
...
def request_repair_plan(
    ...
    *,
    ...
    """Ask a configured bounded model provider for a repair proposal."""

    if not model_available(openai_api_key=api_key, openai_model=model):
        raise StewardError(
            "AI repair mode requires OpenAI credentials or the bounded Copilot CLI provider"
        )
...
def verify() -> tuple[CheckResult, ...]:
    ...
    for audit in run_specialist_audits(ROOT):
        checks.append(
            CheckResult(
                role=audit.name,
                name=audit.packet.task,
...
def _read_report(path: Path) -> AuditReport:
    ...
    return AuditReport(
        ...
        evidence_packets=tuple(data.get("evidence_packets", [])),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "repair":
            ...
            if not model_available(openai_api_key=api_key, openai_model=model):
                print(
                    "AI repair skipped: no configured model provider is available.",
                    file=sys.stderr,
                )
                return 0
